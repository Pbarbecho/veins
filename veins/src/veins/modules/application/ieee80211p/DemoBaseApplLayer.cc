...
void DemoBaseApplLayer::handlePositionUpdate(cObject* obj)
{
    ...
    if (FindModule<TraCIMobility*>::findSubModule(getParentModule())){
          ...
          if (!AddedVehicles && getParentModule()->getIndex() == 0 && simTime() == 5){
              Density();
          }
    }
}
...
void DemoBaseApplLayer::handleLowerMsg(cMessage* msg)
{
    ...
    if (DemoSafetyMessage* bsm = dynamic_cast<DemoSafetyMessage*>(wsm)) {
        ...
        // if (node_name == "rsu"){controlMessage(bsm, "rx");}
...
void DemoBaseApplLayer::Density(){
    AddedVehicles = true;
    for (int i=1;i<NumberofVehicles;i++){Add_FFV(i);}
}

void DemoBaseApplLayer::Add_FFV(int vehID){
    std::string nodeID = std::to_string(vehID);
    std::string vtype = "passenger"; //vtype
    std::string routeId = std::to_string(RandomNumber());               // Random route for the new vehicle (routes are defined in ,rou)
